...
describe('CreateSaleUseCase', () => {
  ...
  describe('Successful Sale Creation', () => {
    ...
    /*   it('should handle customer with company name', async () => {
      const customer = createCustomerMock({
        user: {
          id: 'user-id',
          type: 'COMPANY',
      ...
      (await createSaleUseCase.execute(input)) as any;

      expect(storageService.uploadFile).toHaveBeenCalledWith({
        buffer: mockFile.buffer,
        key: expect.stringMatching(/^receipts\/Test Company LTDA_.*\.pdf$/),
        contentType: 'application/pdf',
      });
    });

    it('should handle customer with individual name', async () => {
      const customer = createCustomerMock({
        user: {
          id: 'user-id',
          type: 'INDIVIDUAL',
      ...
      expect(storageService.uploadFile).toHaveBeenCalledWith({
        buffer: mockFile.buffer,
        key: expect.stringMatching(/^receipts\/John Doe_.*\.pdf$/),
        contentType: 'application/pdf',
      });
    });

    it('should handle customer with no name (fallback to "any")', async () => {
      const customer = createCustomerMock({
        user: {
          id: 'user-id',
          type: 'INDIVIDUAL',
      ...
      expect(storageService.uploadFile).toHaveBeenCalledWith({
        buffer: mockFile.buffer,
        key: expect.stringMatching(/^receipts\/any_.*\.pdf$/),
        contentType: 'application/pdf',
      });
    }); */
  });
